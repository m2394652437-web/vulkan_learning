    ...
    const PipelineConfigInfo& configInfo)
{
    ...
    //如何解释vertex buffer data(图形管线的初始输入)
...
void LvePipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo)
{
    ...
    //指任何三个点会组成一个三角形
    //用于在使用带状拓扑变体的时候，通过在索引缓冲区插入特殊的索引来拆分带状拓扑
    ...
    //光栅化
    //将几何体分解为片段，每个像素或三角形重叠的部分
    configInfo.rasterizationInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;
    //强制gl_Position的z分量位于0~1, 小于0的变为0,大于1的变为1
    //启用此功能需要用到GPU功能
    configInfo.rasterizationInfo.depthClampEnable = VK_FALSE;
    //在光栅化前丢弃所有图元
    //在只需要前几个阶段的图形管线时才需要这个功能
    configInfo.rasterizationInfo.rasterizerDiscardEnable = VK_FALSE;
    //绘制角，边还是填充的三角形
    configInfo.rasterizationInfo.polygonMode = VK_POLYGON_MODE_FILL;
    configInfo.rasterizationInfo.lineWidth = 1.0f;
    //选择根据三角形的朝向(缠绕顺序)来丢弃三角形，以显著提升性能
    configInfo.rasterizationInfo.cullMode = VK_CULL_MODE_NONE;
    //用于指定顺时针还是逆时针作为正面， 以此配合cullmode来丢弃三角形
    configInfo.rasterizationInfo.frontFace = VK_FRONT_FACE_CLOCKWISE;
    configInfo.rasterizationInfo.depthBiasEnable = VK_FALSE;
    //深度偏移
    //通过添加常量或者乘以片段斜率的倍数来改变深度值
